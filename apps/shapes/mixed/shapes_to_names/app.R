...
# Combine shapes and metadata
shapes <- c(shapes_2d, shapes_3d)

shapes_info <- c(shapes_info_2d, shapes_info_3d)
...
# Dimension lookup: 2d or 3d
shape_dim <- c(
  setNames(rep("2d", length(shapes_2d)), names(shapes_2d)),
  setNames(rep("3d", length(shapes_3d)), names(shapes_3d))
)
...
"
    ...
    tags$script(HTML(
      "
document.addEventListener('DOMContentLoaded', function () {
  console.log('audio-helper: DOMContentLoaded (mixed)');
...
server <- function(input, output, session) {
  ...
  audio_term_html <- function(article, noun, preferred_flag, dim) {
    enable_audio <- !(dim == "3d" && isFALSE(preferred_flag) && grepl("seitige", noun, fixed = TRUE))
    art_esc <- htmltools::htmlEscape(article)
    noun_esc <- htmltools::htmlEscape(noun)
    base_text <- sprintf("<b>%s %s</b>", art_esc, noun_esc)
    if (!enable_audio) return(base_text)
    
    words <- strsplit(noun, "\\s+")[[1]]
    base <- if (dim == "2d") "audio/2d/" else "audio/3d/"
    if (length(words) == 1) {
    ...
    if (mode() == "check") {
      ...
      if (length(match_idx) == 0) {
        ...
        pref_parts <- vapply(
          ...
          function(i) audio_term_html(preferred$article[i], preferred$noun[i], preferred$preferred[i], dim),
          character(1)
        )
        ...
        if (length(correct_article_idx) == 0) {
          ...
          if (!chosen$preferred) {
            ...
            chosen_html <- audio_term_html(chosen$article, chosen$noun, chosen$preferred, dim)
            ...
            canonical_html <- audio_term_html(canonical_article, canonical_noun, TRUE, dim)
            
            extra_note <- ""
            if (key == "tri_pyramid") {
              other <- forms[forms$noun != canonical_noun & forms$preferred, , drop = FALSE]
              if (nrow(other) == 1) {
                other_html <- audio_term_html(other$article, other$noun, other$preferred, dim)
                extra_note <- paste0(
