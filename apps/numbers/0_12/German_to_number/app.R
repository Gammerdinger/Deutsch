...
"
  ...
  # Center all content; keep vertical spacing compact for embedding in a short iframe
  div(
...
server <- function(input, output, session) {
  ...
  output$german_word_ui <- renderUI({
    ...
    # Audio subdirectory (0–9 vs 10–12)
    audio_subdir <- if (num <= 9) "0_9" else "10_19"
    
    # In both local Shiny and shinylive, files under www/ are served at root.
    # Our audio files live in www/audio/numbers/...
