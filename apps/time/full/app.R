...
german_time_options_display <- function(hours12, minutes, is_pm) {
  ...
  if (minutes == 0) {
    # accept Mitternacht / Mittag as extra alternatives
    if (h24_num == 0) {
...
# ---------- UI (centered single-column layout) ----------
ui <- fluidPage(
  tags$head(
    tags$style(HTML("
      .umlaut-buttons {
    ...
    tags$script(HTML("
      ...
      // Event delegation so it works for dynamically-created elements
...
server <- function(input, output, session) {
  ...
  # Valid answers (all forms, including special cases & Mitternacht/Mittag)
